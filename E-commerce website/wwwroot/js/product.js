...
const scentBtns = document.querySelectorAll('.scent-radio-btn'); // selecting size buttons
let checkedBtn = 0; // current selected button

scentBtns.forEach((item, i) => { // looping through each button
    item.addEventListener('click', () => { // adding click event to each 
        scentBtns[checkedBtn].classList.remove('check'); // removing check class from the current button
